...
def gradient_only_train(model, X, Y, latent_dim=20, n_iter=5000):
    ...
    for i in range(5000 + 1):
        ...
        with torch.no_grad():
            x = torch.sigmoid(F.linear(X_train, params[0], params[1]))
            x = torch.sigmoid(F.linear(x, params[2], params[3]))
            pred = F.linear(x, params[4], params[5])
            current_loss = F.mse_loss(pred, Y_train).item()
        losses.append(current_loss)
        
        # SPSA gradient estimation
        delta = torch.randint(0, 3, (latent_dim,), device=device).float() - 1
        N_plus = N + c * delta
        N_minus = N - c * delta
        ...
        with torch.no_grad():
            x_plus = torch.sigmoid(F.linear(X_train, params_plus[0], params_plus[1]))
            x_plus = torch.sigmoid(F.linear(x_plus, params_plus[2], params_plus[3]))
            pred_plus = F.linear(x_plus, params_plus[4], params_plus[5])
            loss_plus = F.mse_loss(pred_plus, Y_train).item()
            
            x_minus = torch.sigmoid(F.linear(X_train, params_minus[0], params_minus[1]))
            x_minus = torch.sigmoid(F.linear(x_minus, params_minus[2], params_minus[3]))
            pred_minus = F.linear(x_minus, params_minus[4], params_minus[5])
            loss_minus = F.mse_loss(pred_minus, Y_train).item()
